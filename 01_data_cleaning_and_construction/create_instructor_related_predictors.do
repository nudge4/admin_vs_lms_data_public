/* This script creates all instructor-related predictors, including full-time status, indicator for tenure, the average grade assigned to the course by each instructor, for all non-first-term observations */
...
forvalues y=2008/2020 {
	foreach v in "3_spe" "4_sue" "6_fae" {
		preserve
			...
			duplicates drop
			merge m:1 vccsid_instructor college using "~\Box Sync\Clickstream\data\full\instructor_list.dta", keep(3) nogen
			tempfile tmp
			save "`tmp'", replace
		restore
...
duplicates drop
merge 1:m vccsid_instructor college using "~\Box Sync\Clickstream\data\full\instructors.dta", nogen
drop course
duplicates drop
...
duplicates drop
merge 1:m vccsid_instructor college teaching_strm using "~\Box Sync\Clickstream\data\full\instructors.dta", nogen
...
foreach t in 2193 2194 2203 2204 2212 {
	preserve
		use "~\Box Sync\Clickstream\data\full\instructors.dta", clear
		drop tenure section
		duplicates drop
...
save "~\Box Sync\Clickstream\data\full\taught_before.dta", replace
...
preserve
	use "~\Box Sync\Clickstream\data\full\taught_before.dta", clear
...
merge 1:m strm college subject course_num using "~\Box Sync\Clickstream\data\full\taught_before.dta", keep(2 3) nogen
...
duplicates drop
merge 1:m vccsid_instructor college teaching_strm course using "~\Box Sync\Clickstream\data\full\instructors.dta", keep(2 3) nogen
...
merge m:1 strm college course section using "~\Box Sync\Clickstream\data\full\full_time.dta", nogen
...
save "~\Box Sync\Clickstream\data\full\instructor_related_predictors.dta", replace
