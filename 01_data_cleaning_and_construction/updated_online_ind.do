/* This script creates the indicators for online and in-person courses in the study sample for non-first-term observations */

use "~\Box Sync\Clickstream\data\full\all_vccsid.dta", clear
...
keep vccsid college strm subject course_num section instmod
duplicates drop
sort vccsid college strm subject course_num section instmod
...
*/
gen online_ind = 0
replace online_ind = 1 if inlist(instmod, "CV", "ER", "WW", "HY")
gen inperson_ind = 0
replace inperson_ind = 1 if instmod == "P"
drop instmod
gen course = subject + "_" + course_num
drop subject course_num
sort strm college course section vccsid
merge 1:1 strm college course section vccsid using "~\Box Sync\Clickstream\data\full\LMS_data_new.dta", keep(2 3) nogen
drop grade-tot_time_qrt1
sort strm college course section vccsid
replace online_ind = 0 if online_ind == .
replace inperson_ind = 0 if inperson_ind == .
save "~\\Box Sync\\Clickstream\\data\\full\\updated_online_ind.dta", replace
