...
and also identify the concurrent courses of each course x section in the study sample for all non-first-term observations */
...
preserve
	...
	sort vccsid strm
	save "~\Box Sync\Clickstream\data\full\updated\additional_term_specific.dta", replace
restore
merge m:1 vccsid strm using "~\Box Sync\Clickstream\data\full\all_vccsid_strm.dta", keep(2 3) nogen
drop dev *_share
...
drop course_counts
sort vccsid strm college course
save "~\Box Sync\Clickstream\data\full\concurrent_courses.dta", replace
