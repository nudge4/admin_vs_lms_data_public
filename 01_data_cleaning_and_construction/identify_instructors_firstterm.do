/* This script identifies all VCCS instructors who have taught the courses included in the study sample of all first-term observations */
...
preserve
	zipuse "~\Box Sync\VCCS restricted student data\Build\Course\Course_2020_4_sue.dta.zip", clear
	tempfile su
	save "`su'", replace
restore
append using "`su'", force
...
preserve
	...
	duplicates drop
	save "~\Box Sync\Clickstream\data\first\full_time.dta", replace
restore
...
duplicates drop
save "~\Box Sync\Clickstream\data\first\instructor_list.dta", replace
