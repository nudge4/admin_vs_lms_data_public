# This script fits the full-predictor RF model on all first-term observations without imputation of missing values -- just naturally treat missing values as one separate category.
...
df0 = pd.read_stata("LMS_data_final.dta")
...
l5 = ['has_assign_sub_cnt_qtr1', 'has_on_time_assign_share_qtr1', 'has_on_time_assign_share_qtr1c', 'has_assign_sub_cnt_qtr1c', 'has_avg_g_concurrent', 'has_past_avg_grade']
for p in l5:
    v = np.min(df[p.replace("has_", "")]) - 1
    df.loc[:, p.replace("has_", "")] = df.apply(lambda x: v if x[p] != 1 else x[p.replace("has_", "")], axis=1)
predictors = [p for p in predictors if p not in set(l5+['has_concurrent_qtr1'])]
