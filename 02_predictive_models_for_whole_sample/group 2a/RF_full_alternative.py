# This script fits the full-predictor RF model on all non-first-term observations without imputation of missing values -- just naturally treat missing values as one separate category.
...
l5 = ['has_assign_sub_cnt_qtr1', 'has_on_time_assign_share_qtr1', 'has_on_time_assign_share_qtr1c', 'has_assign_sub_cnt_qtr1c', 'has_repeat_grade', 'has_prereq_grade', 'has_avg_g_concurrent', 'has_term_gpa_1', 'has_term_gpa_2', 'has_past_avg_grade']
for p in l5:
...
predictors = [p for p in predictors if p not in set(l4+l5+l6+['has_concurrent_qtr1', 'prior_has_qtr1', 'prior_has_full'])]
...
pr_rf = create_confusion_matrix(y_test_pred_rf, best_threshold, "RF_full_cm")
print(pr_rf)